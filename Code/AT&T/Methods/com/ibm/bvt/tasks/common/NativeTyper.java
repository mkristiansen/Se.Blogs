...
public class NativeTyper {
	...
	private void doType(int... keyCodes) {
		...
		keyPressNativeSequence(keyCodes, 0, keyCodes.length);

	}

	private void getKeyCode(char character) {
		switch (character) {
		case 'a':
		...
		case ' ':
			...
			break;
		default:
			System.out.println("Warning: Cannot type character " + character);
		}
	}

	private void keyDownNative(String keyCode) {
		CommonMethods.selenium.keyDownNative(keyCode);
	}

	private void keyPressNativeSequence(int[] keyCodes, int offset, int length) {
		if (length != 0) {
			keyDownNative(Integer.toString(keyCodes[offset]));
			keyPressNativeSequence(keyCodes, offset + 1, length - 1);
			keyUpNative(Integer.toString(keyCodes[offset]));
		} else {
			return;
		}
	}

	private void keyUpNative(String keyCode) {
		CommonMethods.selenium.keyUpNative(keyCode);
	}

	// Note: You must select and focus correct window before using nativeType
	public void nativeType(String text) {
